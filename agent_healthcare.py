...
async def get_flow_dict(agent_id: str) -> dict:
    async with httpx.AsyncClient() as client:
        response = await client.get(f"https://72be-88-86-246-30.ngrok-free.app/agents/{agent_id}")
    agent = response.json()
    return {card["id"]: card for card in agent["milestone_cards"]}
...
class Assistant(Agent):
    def __init__(self) -> None:
        super().__init__(
            instructions=(
                "You are a doctor in a healthcare call center. "
                "Keep responses concise and natural for spoken audio."
            ),
        )

    async def on_enter(self) -> None:
        # Reset + seed chat context ("memory") for this conversation.
        # NOTE: The following are *simulation assumptions for testing*, not real-world facts.
        chat_ctx = llm.ChatContext.empty()
        chat_ctx.add_message(
            role="system",
            content=dedent(
                """
                Simulation assumptions for this conversation:
                - Today is March 1, 2025.
                - The United States president is Donald Trump.
                - Ali Khamenei died today.

                These are verified real-world facts.
                """
            ).strip(),
        )
        ...
        next_card = "card1"
        last_output = None
        flow = await get_flow_dict("69a3b743e499c30ea9bc5794")
        while not end_conversation:
            card = flow[next_card]
            if card["action"] == "expect_options":
                selected_option = await ExpectOptionsTask(
                    question=card["question"],
                    options=card["expectedOptions"],
                    chat_ctx=self.chat_ctx,
                )
                last_output = selected_option
                next_card = next(
                    (c["next"] for c in card["optionRoutes"] if c["option"] == selected_option),
                    None  # default if no match
                )
                if next_card is None:
                    raise ValueError(f"Missing graph card. No next card found for option: {selected_option}")
                await emit_event(
                    {
                        "type": "milestoneCompleted",
                        "cardId": card["id"],
                        "action": "expectOptions",
                        "optionCaptured": selected_option,
                        "nextCard": next_card,
                    }
                )
            else:
        ...
        if next_card != "card5":
            if next_card == "card3":
                await InformUserCriticalTask(
                    info="You are having severe chest pain. Please call 911 immediately.",
                    chat_ctx=self.chat_ctx,
                )
            else:
                await InformUserBasicTask(
                    info="You are having mild chest pain. Please take a deep breath and relax.",
                    chat_ctx=self.chat_ctx,
                )
        else:
            await InformUserBasicTask(
                info="You are not having chest pain. Please continue with your day.",
                chat_ctx=self.chat_ctx,
            )
...
@server.rtc_session(agent_name="69a3b743e499c30ea9bc5794")
